...
@section('content')
  <div class="page-header">
    <h4 class="page-title">{{ __('Registered Partner') }}</h4>
    <ul class="breadcrumbs">
      ...
      <li class="nav-item">
        <a href="#">{{ __('Registered Partner') }}</a>
      </li>
    </ul>
  </div>
    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <div class="row">
            <div class="col-4">
              <div class="card-title">{{ __('All Partners') }}</div>
            </div>
            ...
            <div class="col-4">
                <form action="{{ route('admin.vendor_management.registered_vendor') }}" method="get"
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <select name="package_id" id="" class="select2"
                                onchange="document.getElementById('packageSearch').submit()">
                                <option value="" selected>{{ __('All') }}</option>
                                @foreach ($packages as $package)
                                    <option @selected($package->id == request()->input('package_id')) value="{{ $package->id }}">
                        ...
                        <div class="col-lg-6">
                            <input type="text" name="expire_range" id="expireRange" class="form-control"
                                value="{{ request('expire_range') }}" placeholder="Select Expire Date Range" autocomplete="off">
                        </div>
                  ...
                  <table class="table table-striped mt-3">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">{{ __('Current Package') }}</th>
                        <th scope="col">{{ __('Package Expire') }}</th>
                    ...
                    <tbody>
                      @foreach ($vendors as $vendor)
                        ...
                        <tr>
                          ...
                          <td>{{ $currPackage ? $currPackage->title : '' }}</td>
                          <td> {{ $currMemb ? Carbon\Carbon::parse($currMemb->expire_date)->format('d-m-Y') : ''}}</td>
                          <td>
