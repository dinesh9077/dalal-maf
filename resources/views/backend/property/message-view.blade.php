...
<div class="modal fade" id="changeStatusModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">{{ __('Change Status') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>

            <form class="ajaxForm" class="modal-form create" action="{{ route('admin.inquiry.change_status') }}" method="POST">
              @csrf
              <input type="hidden" name="inquiry_id" id="in_id" value="">
              ...
              <div class="modal-body">
                  <div class="row no-gutters">
                      <div class="col-lg-12">
                          <div class="form-group">
                              <label>Status</label>
                              <select class="form-control" name="approve_status" id="in_status" required>
                                  @foreach ($statuses as $status)
                                      <option value="{{ $status->id }}">{{ $status->name }}</option>
                                  @endforeach
                              </select>
                              <p id="err_approve_status" class="mt-2 mb-0 text-danger em"></p>
                          </div>
                      </div>
                      ...
                      <div class="col-lg-12">
                          <div class="form-group">
                              <label>Comment</label>
                              <textarea rows="4" class="form-control" name="comment" id="in_comment" required ></textarea>
