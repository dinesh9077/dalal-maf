...
@if($floors != 0)
<form id="propertyFlatAddForm" class="property-card " action="{{ route('admin.property_inventory.flat.store') }}" method="post" enctype="multipart/form-data">
    <div class="floor-row">
        @csrf
        ...
        @for($i = 1; $i <= $floors; $i++)
            ...
            <div class="floor-wrapper mt-3">
                <div class="pro-flor-row filter-deta">
                    <div class="row row-floor mt-2 new-property-box align-items-end">

                        <div class="col-md">
                            <p class="font-black-12 mb-2 property-title">Flat/Shop/Raw H. No</p>
                            <input type="text" class="form-control new-controls-property"
                                name="flat_no[]" id="flat_no{{ $i }}"
                                value="{{ $partailsDetail ? $partailsDetail->flat_no : $floor_number.$j }}" required>
                        </div>

                        <div class="col-md">
                            <p class="font-black-12 mb-2 property-title">Unit</p>
                        ...
                        <div class="col-md">
                            <p class="font-black-12 mb-2 property-title">Sq.ft/Carpet Area <span class="text-danger">*</span></p>
                            <input type="text" class="form-control new-controls-property"
                                name="sqft[]" id="sqft{{ $i }}"
                                value="{{ $partailsDetail ? Helper::decimalsprint($partailsDetail->sqft, 2) : '' }}"
                                oninput="$(this).val($(this).val().replace(/[^0-9.]/g, ''));" required>
                        </div>

                        {{-- <div>
                            <p class="font-black-12 p-0 m-0 border-0 mb-2 property-title">Plot size/square yard<span class="text-danger">*</span></p>
                            <input type="text" class="modal-input new-controls-property" name="plot_size[]" id="plot_size{{ $i }}" value="{{ $partailsDetail ? $partailsDetail->plot_size : '' }}" required>
                    </div> --}}
                    {{-- <div>
                            <p class="font-black-12 p-0 m-0 border-0 mb-2 property-title">Budget <span class="text-danger">*</span></p>
                            <select class="form-control select2 new-controls-property" id="budget{{ $i }}" name="budget[]" required>
                    <option value="">Select Budget</option>
                    <option value="100-1k">{{ __('100 - 1 K') }}</option>
                    <option value="1k-5k">{{ __('1 k - 5 K') }}</option>
                    <option value="5k-10k">{{ __('5 k - 10 K') }}</option>
                    </select>
                </div> --}}

                <div class="col-md">
                    <p class="font-black-12 mb-2 property-title">Price <span class="text-danger">*</span></p>
                    <input type="text" class="form-control new-controls-property"
                        name="price[]" id="price{{ $i }}"
                        value="{{ $partailsDetail ? Helper::decimalsprint($partailsDetail->price, 3) : '' }}"
                        oninput="$(this).val($(this).val().replace(/[^0-9.]/g, ''));" required>
                </div>

                {{-- <div class="col-md">
                    <p class="font-black-12 mb-2 property-title">Property Status <span class="text-danger">*</span></p>
                    <select class="form-control new-controls-property"
                        name="property_status[]" id="property_status{{ $i }}"
                        data-property-id="{{$partailsDetail->property_id ?? ''}}"
                        data-unit-id="{{$partailsDetail->id ?? ''}}"
                        data-wing-id="{{$partailsDetail->wing_id ?? ''}}"
                        data-floor-id="{{$partailsDetail->floor_id ?? ''}}"
                        required onchange="openWorkModel(this)">
            ...
            <!-- <div class="row-floor mt-2 new-property-box">
                        <div>
                            <p class="font-black-12 p-0 m-0 border-0 mb-2 property-title">Flat/Shop/Raw h. No</p>
                            <input type="text" class="modal-input new-controls-property" name="flat_no[]" id="flat_no{{ $i }}"  value="{{ $partailsDetail ? $partailsDetail->flat_no : $floor_number.$j }}" required>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" class="modal-input new-controls-property" name="sqft[]" id="sqft{{ $i }}" value="{{ $partailsDetail ? Helper::decimalsprint($partailsDetail->sqft, 2) : '' }}" oninput="$(this).val($(this).val().replace(/[^0-9.]/g, ''));" required>
                        </div>
                        <div>
                            <p class="font-black-12 p-0 m-0 border-0 mb-2 property-title">Price <span class="text-danger">*</span></p>
                            <input type="text" class="modal-input new-controls-property" name="price[]" id="price{{ $i }}"   oninput="$(this).val($(this).val().replace(/[^0-9.]/g, ''));" value="{{ $partailsDetail ? Helper::decimalsprint($partailsDetail->price, 3) : '' }}" required>
                        </div>
                        <div>

                            <p class="font-black-12 p-0 m-0 border-0 mb-2">Property Status <span class="text-danger">*</span></p>
                            <select class="form-control" name="property_status[]" id="property_status{{ $i }}" data-property-id="{{$property_id}}" data-unit-id="{{$partailsDetail->id ?? ''}}" data-wing-id="{{$wing_id}}" data-floor-id="{{$floor_id}}" required onchange="openWorkModel(this)">
...
<script>
    ...
    function removeDetailsRow(details_id, floors, floor_number, wing_id, floor_id) {
        if (confirm('Are you sure to delete?')) {
            $.post("{{route('admin.property_inventory.flat.delete')}}", {
                    _token: "{{csrf_token()}}",
