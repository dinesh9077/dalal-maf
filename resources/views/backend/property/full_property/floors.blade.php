...
<script>
	...
	function floorSubmit(obj, event) {
		...
		$.post("{{route('admin.property_inventory.floor.store')}}", {
				_token: "{{csrf_token()}}",
	...
	function loadPartial(floors, floor_number, wing_id, floor_id) {
		$.post("{{route('admin.property_inventory.floor.partial')}}", {
      _token: "{{csrf_token()}}",
      property_id: property_id,
      floors: floors,
      wing_id: wing_id,
	...
	function removeRowFloor(floor_id, wings, wing_number, wing_id) {
		if (confirm('Are you sure to delete?')) {
			$.post("{{route('admin.property_inventory.floor.delete')}}", {
					_token: "{{csrf_token()}}",
					property_id: property_id,
					wings: wings,
					wing_id: wing_id,
	...
	function copyAllRowsAndFloors(obj, event) {
		...
		Swal.fire({
			...
			t.value &&
				...
				$.post("{{url('admin/property-inventory/copy/floors')}}/" + floor_id, {
					_token: "{{csrf_token()}}"
				}, function(res) {
					if (res.status == "error") {
