...
<div class="add-modal-main modal fade" id="property_work_modal" data-backdrop="static" data-keyboard="false" tabindex="-1"
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"
                    style="font-size : 24px; color : #000000; border : none; background : white;">×</button>
            </div>
            <form id="propertyWorkAddForm" action="{{ route('admin.property_inventory.floor.status_store') }}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6" style="pointer-events:none;">
                                ...
                                <select name="property_id" class="form-control" id="property_ids">
                                    <option value="{{ $property->property_id }}" <?php echo $property->id == $property_id ? 'selected' : ''; ?>>
                            ...
                            <div class="col-lg-6" style="pointer-events:none;">
                                ...
                                <select name="wing_id" class="form-control" id="wing_id">
                                    ...
                                    @foreach ($wings as $wing)
                                        <option value="{{ $wing->id }}" <?php echo $wing->id == $wing_id ? 'selected' : ''; ?>>{{ $wing->wing_number }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-6" style="pointer-events:none;">
                                ...
                                <select name="floor_id" class="form-control" id="floor_id">
                                    ...
                                    @foreach ($floors as $floor)
                                        <option value="{{ $floor->id }}" <?php echo $floor->id == $floor_id ? 'selected' : ''; ?>>
                                            {{ $floor->floor_number }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-6" style="pointer-events:none;">
                                <label class="label-main">Select Flat <span class="text-danger">*</span></label>
                                <select class="form-control" name="unit_id" id="unit_id">
                                    <option value="">Select Flats</option>
                                    @foreach ($floor_details as $floor_detail)
                                        <option value="{{ $floor_detail->id }}" <?php echo $floor_detail->id == $unit_id ? 'selected' : ''; ?>>
                                            {{ $floor_detail->flat_no }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
        ...
        $('#propertyWorkAddForm').submit(function(event) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    $('#propertyWorkAddForm').find('button.spin-button').removeClass('loading').html(
                        'Save');
                    if (res.status == "error") {
