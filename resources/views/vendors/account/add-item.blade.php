...
<tr class="row_id_{{$i}}">
	<td>
		<input type="hidden" class="form-control" name="item_id[{{$i}}]" value="" id="item_id_{{$i}}" required>
	...
	<td>
		<input name="details[{{$i}}]" id="details_{{$i}}" class="form-control" rows="1" placeholder="Enter Item Description" required>
	</td>
	<td>
		<input class="form-control invo_price text-right" data-id="{{$i}}" placeholder="Price" type="text" name="price[{{$i}}]" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"  value="0" id="price_{{$i}}">
	</td>
	<td>
		<input class="form-control invo_qty" placeholder="Qnty." type="text" name="quantity[{{$i}}]" value="1" id="qty_{{$i}}" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" readOnly>
	</td>
	<td>
    <input type="text" name="unit[{{$i}}]" id="unit_{{$i}}" readOnly>
	...
	<td class="text-right">
		<button type="button" class="main-btn delet-buss delet-btn remove_row" href="javascript:;" id="{{$i}}" style="cursor:pointer" title="Remove row"><i class="mdi mdi-delete-outline" aria-hidden="true"></i></button>
	</td>
</tr>
...
<tr class="row_id_{{$i}}">
	...
	<td>
		<input type="text" id="discount_{{$i}}" name="discount[{{$i}}]" value="0" class="form-control invo_discount" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"  >
	</td>
