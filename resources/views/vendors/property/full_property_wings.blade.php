		...
		<div class="wing-add-main" id="addMoreWings">
			...
			<div class="card-radio d-flex mt-3 ">
				...
				<div class="pro-input-add">
					...
					<a href="javascript:;" class="main-btn add-image-btn add-pro-modal ml-2 wing-add recycle-icons-save" id="main_save_{{($key+1)}}" data-wing-id="{{$wing->id}}" data-number="{{($key+1)}}" onclick="wingSubmit(this,event,1)" style="display:none;">Save</a>
				</div>
			</div>
			@endforeach
			@else
			<div class="card-radio d-flex mt-2">
				<div class="wings-radio">
					<ul class="new-ul-des-start">
...
<script>
	...
	function wingSubmit(obj, event, num = null) {
		...
		$.post("{{route('vendor.property_inventory.wings.store')}}", {
				_token: "{{csrf_token()}}",
	...
	function loadFloors(wings, wing_number, wing_id) {
		...
		$.post("{{route('vendor.property_inventory.wings-floors')}}", {
				_token: "{{csrf_token()}}",
				property_id: property_id,
				wings: wings,
				wing_id: wing_id,
	...
	function removeRowWing(wing_id)
	{
		...
		{
			$.post("{{route('vendor.property_inventory.wing-delete')}}",
			{
				_token:"{{csrf_token()}}",
				property_id:property_id,
				wing_id:wing_id
