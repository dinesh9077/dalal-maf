...
@if($floors != 0)
<form id="propertyFlatAddForm" class="property-card " action="{{ route('vendor.property_inventory.flat.store') }}" method="post" enctype="multipart/form-data">
	<div class="floor-row">
        @csrf
...
<script>
	...
	function removeDetailsRow(details_id,floors,floor_number,wing_id,floor_id)
	{
		...
		{
			$.post("{{route('vendor.property_inventory.flat.delete')}}",
			{
				_token:"{{csrf_token()}}",
