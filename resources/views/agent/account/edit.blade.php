...
<div class="page-content">
	<form method="post" action="{{route('account.sales-bill.update',['id'=>$saleInvoice->id])}}" id="invoice_update_form" enctype="multipart/form-data">
		{{ csrf_field() }}
		<div class="import-row-main">
			<div class="invoice-main filter-deta">
				<div class="card-box">
									...
									<div class="row">
										<div class="col-12">
											<div class="form-group">
												...
												<input class="modal-input" type="text" name="invoice_number" id="invoice_number" value="{{$saleInvoice->invoice_number}}">
												<input class="modal-input" type="hidden" name="prefix" value="{{$saleInvoice->prefix}}">
											</div>
										</div>
										<div class="col-lg-6">
											<div class="form-group">
				...
				<div class="card-box">
					<div class="invoice-middle-main">
                        ...
                        <input type="hidden" name="status_item_id[{{$key}}]" value="{{$invoiceItem->item_id }}" id="item_id_{{$i}}">
                              ...
                              @foreach($propertyStatus as $prtStatus)
                                  ...
                                  <option value="{{ $prtStatus->property_id }}" <?php echo ($prtStatus->id == $invoiceItem->item_id)?'selected':''; ?> data-item-id="{{ $invoiceItem->item_id }}" data-price="{{ $prtStatus->Units->price}}" data-unit="{{ $prtStatus->Units->unit->unit_name}}" data-description="{{ trim($wingName) }}/{{ trim($floorName) }}/{{ $flatNo }}">
                              ...
                              @endforeach
													...
													<td>
                            <input type="text" name="unit[{{$key}}]" id="unit_{{$key}}" value="{{$invoiceItem->unit}}" readOnly>
													...
													<td class="text-right">
														<button type="button" class="main-btn delet-buss delet-btn delet-btn remove_row" href="javascript:;" id="{{$i}}" style="cursor:pointer" title="Remove row"><i class="mdi mdi-delete-outline" aria-hidden="true"></i></button>
													</td>
												</tr>
				...
				<div class="card-box">
					<div class="top-invoice-main">
						...
						<div class="footer-area">
							<textarea class="modal-input" name="footer_note" rows="2" placeholder="Enter a footer for this invoice (eg. Tax info, Thank you note, etc.)">{{$saleInvoice->footer_note}}</textarea>
						</div>
					</div>
				</div>
