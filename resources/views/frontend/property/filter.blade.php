...
<head>
  ...
  <style>
    body { margin:0; background:#f9f9f9; font-family: 'Open Sans', system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; }
    /* Banner container */
    .home-banner { position:relative; overflow:visible; padding: 12px 0 24px; }
    .banner-filter-form { margin:0 auto; width: min(1200px, 95%); background:#fff; border-radius: 12px; box-shadow: 0 8px 24px rgba(0,0,0,.08); padding: 16px; }
    /* Tabs + post property btn */
    .tabs-wrapper { border-bottom:1px solid #f4f5f7; display:flex; align-items:center; justify-content:space-between; gap:12px; }
    .nav-tabs { border:0; gap:6px; }
    .nav-tabs .nav-link { border:0; color:#6c603c; padding:10px 12px; }
    .nav-tabs .nav-link.active { border-bottom:2px solid #6c603c; font-weight:600; color:#6c603c; background:transparent; }
    .style__postContainerTab { display:inline-flex; align-items:center; gap:8px; padding:8px 14px; border-radius:8px; background:#6c603c; color:#fff; text-decoration:none; border:none; }
    .style__postContainerTab:hover { background:#5a5231; color:#fff; }
    /* Grid */
    .form-wrapper .grid { display:grid; grid-template-columns: repeat(5, 1fr); gap:12px; }
    .grid-item.home-des-border { border:1px solid #e5e5e5; border-radius:8px; padding:12px; background:#fff; }
    .form-group { display:flex; flex-direction:column; gap:6px; }
    .form-group label { font-weight:600; color:#6c603c; font-size:14px; }
    /* Price */
    .price-value { font-weight:600; color:#6c603c; font-size:14px; display:inline-block; margin-bottom:8px; }
    [data-range-slider] { position:relative; width:100%; height: 36px; }
    [data-range-slider] .slider-track-base { position:absolute; left:0; right:0; top:50%; transform:translateY(-50%); height:4px; background:#bfb8a2; border-radius:2px; }
    [data-range-slider] .slider-active { position:absolute; top:50%; transform:translateY(-50%); height:4px; background:#6c603c; border-radius:2px; }
    [data-range-slider] input[type="range"] { position:absolute; left:0; bottom:0; width:100%; pointer-events:none; -webkit-appearance:none; background:none; border:none; }
    [data-range-slider] input[type="range"]::-webkit-slider-runnable-track { height:4px; border-radius:2px; background:transparent; border:none; }
    [data-range-slider] input[type="range"]::-webkit-slider-thumb {
      pointer-events:all; width:16px; height:16px; border-radius:50%; background:#fff; border:3px solid #6c603c;
      cursor:pointer; -webkit-appearance:none; margin-top:-8px; position:relative; z-index:2;
    }
    /* Submit button */
    .new-search-btn { background-color:#6c603c !important; border-color:#6c603c !important; border-radius:8px; padding: 10px 16px; display:inline-flex; align-items:center; justify-content:center; }
    .new-icons-search { width:18px; height:18px; display:block; }
    /* Mobile */
    @media (max-width: 1199px) { .form-wrapper .grid { grid-template-columns: repeat(3, 1fr); } }
    @media (max-width: 767px)  {
      .tabs-wrapper { flex-wrap:wrap; gap:8px; }
      .post-property-btn { width:100%; display:flex; justify-content:flex-end; }
      .form-wrapper .grid { grid-template-columns: 1fr; }
      .banner-filter-form { width: min(100%, 100%); }
    }
     .select2-container { width: 100% !important; height:100% !important;}
  </style>
</head>
<body>
            ...
            <div class="grid">
              ...
              <div class="grid-item home-des-border">
                <div class="form-group">
                  ...
                  <select name="category[]" id="category_{{ $key }}" class="form-control select2 bringCategory">
