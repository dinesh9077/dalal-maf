                                    ...
                                    <ul class="share-link list-unstyled">
                                        ...
                                        <li>
                                            ...
                                            @if (!Auth::guard('vendor')->check() && !Auth::guard('web')->check())
                                              <a type="button" class="btn l-s-btn " data-bs-toggle="modal" data-bs-target="#customerPhoneModal" data-action="login">
                            ...
                            <div class="new-details-bg-white">
                              ...
                              <div class="pro-over-main">
                                        <div class="pro-over-detail-box">
                                            ...
                                            <span class="new-future-tit-ans">{{ ucwords(str_replace('_', ' ', $propertyContent->purpose)) }} </span>
                                        </div>
                                        <div class="pro-over-detail-box">
                                            <strong class="pro-over-title">Category</strong>
                                            <span class="new-future-tit-ans">{{ $propertyContent->categoryContent?->name }}</span>
                                        </div>
                                        @if($propertyContent->property->country)
                                        <div class="pro-over-detail-box">
                                            <strong class="pro-over-title">Country</strong>
                                            <span class="new-future-tit-ans">{{ $propertyContent->property->country?->getContent($propertyContent->language_id)?->name }}</span>
                                        </div>
                                        @endif
                                        @if($propertyContent->property->state)
                                        <div class="pro-over-detail-box">
                                            <strong class="pro-over-title">State</strong>
                                            <span class="new-future-tit-ans">{{ $propertyContent->property->state?->getContent($propertyContent->language_id)?->name  }}</span>
                                        </div>
                                        @endif
                                        @if($propertyContent->property->city)
                                        <div class="pro-over-detail-box">
                                            <strong class="pro-over-title">City</strong>
                                            <span class="new-future-tit-ans">{{ $propertyContent->property->city?->getContent($propertyContent->language_id)?->name }}</span>
                                        </div>
                                        @endif
                                        @if($propertyContent->property->areaContent)
                                        <div class="pro-over-detail-box">
                                            <strong class="pro-over-title">Area</strong>
                                            <span class="new-future-tit-ans">{{ $propertyContent->property->areaContent?->name }}</span>
                                        </div>
                                        @endif
                                        <div class="pro-over-detail-box">
    ...
    box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.08);">
                            ...
                            <div class="widget widget-form radius-md mb-30 new-widgets-color" style="background: white !important;border: 1px solid #ced4dd;box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.08);">
                                ...
                                <form action="{{ route('property_contact') }}" method="POST">
                                    ...
                                    @else
                                      <button type="submit"
                                          class="btn btn-md  w-100" style="background-color: #6c603c; color : white;">{{ __('Send inquiry') }}</button>
                                    @endif
                                </form>
                            </div>
                        </aside>
