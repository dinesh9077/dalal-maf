...
class PropertyController extends Controller
{
    public function index(Request $request)
    {
        ...
        $propertyQuery = Property::where('property_type','partial')->where([['properties.status', 1], ['properties.approve_status', 1]])
        ...
        // 2) Get min/max based on filtered result
        $filteredMin = (clone $propertyQuery)->min('properties.price');
        $filteredMax = (clone $propertyQuery)->max('properties.price');

        $information['min'] = intval($filteredMin);
        $information['max'] = intval($filteredMax);

        // 3) Now paginate
        $property_contents = $propertyQuery->paginate(12);
