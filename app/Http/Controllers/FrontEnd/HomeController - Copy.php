<?php
	...
	class HomeController extends Controller
	{
		...
		public function index(Request $request)
		{
			...
			$queryResult['business_for_sale'] = Property::where([['properties.status', 1], ['properties.approve_status', 1]])->where('purpose', 'business_for_sale')->where('property_type','partial')
			->where('property_contents.language_id', $language->id)
			->join('property_contents', 'property_contents.property_id', 'properties.id')
			->join('property_categories', 'property_categories.id', 'properties.category_id')
			->when('properties.vendor_id' != 0, function ($query) {
			...
			->select('properties.*', 'property_contents.language_id', 'property_contents.slug', 'property_contents.title', 'property_contents.address', 'property_contents.language_id')->latest()->get();
			
			$queryResult['franchiese'] = Property::where([['properties.status', 1], ['properties.approve_status', 1]])->where('purpose', 'franchiese')->where('property_type','partial')
			->where('property_contents.language_id', $language->id)
			->join('property_contents', 'property_contents.property_id', 'properties.id')
			->join('property_categories', 'property_categories.id', 'properties.category_id')
			->when('properties.vendor_id' != 0, function ($query) {
			...
			if ($themeVersion == 1 && $secInfo->project_section_status == 1) {
				...
				$queryResult['blogs'] = Blog::where('blogs.status', '=', 1)->join('blog_informations', 'blogs.id', '=', 'blog_informations.blog_id')
			...
			$queryResult['cityCount'] = City::count();
			$queryResult['userCount'] = User::count();
			$queryResult['vendorCount'] = Vendor::count();
