...
use App\Models\Property\FeaturedPropertyInventory;
use App\Models\Property\PropertyInventory;
use App\Models\Property\PropertyAmenity;
use App\Models\Property\PropertyCategory;
use App\Models\Property\PropertyInventoryAmenity;
use App\Models\Property\PropertyInventorySliderImage;
...
class PropertyInventoryController extends Controller
{
    ...
    {
        $data['commertialCount'] = PropertyInventory::where('type', 'commercial')->count();
        $data['residentialCount'] = PropertyInventory::where('type', 'residential')->count();
        return view('backend.property_inventory.type', $data);
    }

    public function settings()
    {
        $content = Basic::select('property_country_status', 'property_state_status')->first();
        return view('backend.property_inventory.settings', compact('content'));
    }
    public function propertSettings()
    {
        $content = Basic::select('property_approval_status')->first();
        return view('backend.property_inventory.property-settings', compact('content'));
    }
    ...
    public function index(Request $request)
    {
        ...
        $data['properties'] = PropertyInventory::join('property_contents', 'properties.id', 'property_contents.property_id')->with([
        ...
        return view('backend.property_inventory.index', $data);
    }
    public function create(Request $request)
    {
        ...
        return view('backend.property_inventory.create', $information);
    }
    public function updateFeatured(Request $request)
    {
        $property = FeaturedPropertyInventory::findOrFail($request->requestId);
    ...
    public function imagesstore(Request $request)
    {
        ...
        $imageName = UploadFile::store(public_path('assets/img/property-inventory/slider-images/'), $request->file('file'));

        $pi = new PropertyInventorySliderImage();
    ...
    {
        $pi = PropertyInventorySliderImage::findOrFail($request->fileid);
        $imageCount = PropertyInventorySliderImage::where('property_id', $pi->property_id)->get()->count();
        if ($imageCount > 1) {
            @unlink(public_path('assets/img/property-inventory/slider-images/') . $pi->image);
            $pi->delete();
            return $pi->id;
        } else {
            return 'false';
    ...
    {
        $pi = PropertyInventory::select('video_image', 'id')->findOrFail($request->fileid);

        if (!empty($pi->video_image)) {
            @unlink(public_path('assets/img/property-inventory/video/') . $pi->video_image);
            $pi->video_image = null;
            $pi->save();
            return 'success';
        } else {
    ...
    {
        $pi = PropertyInventory::select('floor_planning_image', 'id')->findOrFail($request->fileid);

        if (!empty($pi->floor_planning_image)) {
            @unlink(public_path('assets/img/property-inventory/plannings/') . $pi->floor_planning_image);
    ...
    public function store(PropertyStoreRequest $request)
    {
        DB::transaction(function () use ($request) {
            ...
            if (request()->hasFile('featured_image')) {
                $featuredImgName = UploadFile::store(public_path('assets/img/property-inventory/featureds'), $featuredImgURL);
            }
            ...
            if (request()->hasFile('floor_planning_image')) {
                $floorPlanningImage = UploadFile::store(public_path('assets/img/property-inventory/plannings'), $request->floor_planning_image);
            }

            if ($request->hasFile('video_image')) {
                $videoImage = UploadFile::store(public_path('assets/img/property-inventory/video/'), $request->video_image);
            }

            $property = PropertyInventory::create([
            ...
            if ($slders) {
                $pis = PropertyInventorySliderImage::findOrFail($slders);
            ...
            if ($request->has('amenities')) {
                foreach ($request->amenities as $amenity) {
                    PropertyInventoryAmenity::create([
    ...
    public function update(PropertyUpdateRequest $request, $id)
    {
        ...
        DB::transaction(function () use ($request, $id) {
            ...
            if ($request->hasFile('featured_image')) {
                $featuredImgName = UploadFile::update(public_path('assets/img/property-inventory/featureds/'), $request->featured_image, $property->featured_image);
            }
            if ($request->hasFile('floor_planning_image')) {
                $floorPlanningImage = UploadFile::update(public_path('assets/img/property-inventory/plannings/'), $request->floor_planning_image, $property->floor_planning_image);
            }
            if ($request->hasFile('video_image')) {
                $videoImage = UploadFile::update(public_path('assets/img/property-inventory/video/'), $request->video_image, $property->video_image);
            }
            $property->update([
                'vendor_id' => $request->vendor_id ?? 0,
                'agent_id' => $request->agent_id,
    ...
    public function deleteProperty($id)
    {
        ...
        if (!is_null($property->featured_image)) {
            @unlink(public_path('assets/img/property-inventory/featureds/' . $property->featured_image));
        }

        if (!is_null($property->floor_planning_image)) {
            @unlink(public_path('assets/img/property-inventory/plannings/' . $property->floor_planning_image));
        }
        if (!is_null($property->video_image)) {
            @unlink(public_path('assets/img/property-inventory/video/' . $property->video_image));
        }
        $property->proertyAmenities()->delete();
        $propertySliderImages  = $property->galleryImages()->get();
        foreach ($propertySliderImages  as  $image) {

            @unlink(public_path('assets/img/property-inventory/slider-images/' . $image->image));
            $image->delete();
        }
