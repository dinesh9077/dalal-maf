...
class ProjectController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        Project::where('is_new', '0')->update(['is_new' => '1']);
    ...
    { 
		...
		try
		{
			foreach ($propertyIds as $id) {
				$this->deleteProject($id); // Assuming this safely deletes the property and its relations
			}
			DB::commit();
			return response()->json([
				'status' => 'success',
		...
		{ 
			...
			return response()->json([
				'status' => 'error',
				'message' => 'Something went wrong while deleting projects.'.$e->getMessage()
			]);
		}
    }
}
