...
class AccountController extends Controller
{
    ...
    public function salesBillListing()
    {
        ...
        return view('agent.account.sales-billing', compact('prtStatus'));
    }
    ...
    public function index()
    {
        ...
        $customers = Customer::whereIn('id',$customerIds)->where('agent_id',$agentId)->get();

        return view('agent.account.index',compact('customers'));
    }
    ...
    public function salesBillItem($i,Request $request)
    {
      try{
        ...
        $view = view('agent.account.add-item',compact('i','accountUnits','propertyStatus'))->render();
    ...
    public function salesBillInvoiceView($id)
    {
        ...
        return view('agent.account.invoice-view',compact('customer','width','invoice','invoice_items','id','billings'));
      }
      ...
      public function salesBillExport($id)
      {
          ...
          // $pdf = PDF::loadView('agent.account.export_style_1',compact('customer','invoice','invoice_items','id','width','billings'));

         $pdf = PDF::loadView('agent.account.export_style_1', compact('customer', 'invoice', 'invoice_items', 'id', 'width', 'billings', 'amount_due_show'))
      ...
      public function salesBillPayment($invoice_id)
      {
          ...
          $view = view('agent.account.record-payment',compact('invoice_id','saleInvoice','amount_due'))->render();
          return response()->json(['view'=>$view]);
      }
      ...
      public function salesBillViewPayment($invoice_id)
      {
        ...
        $view = view('agent.account.view-payment',compact('billingInvices','paidInvoice','invoice_id'))->render();
        return response()->json(['view'=>$view]);
      }
      ...
      public function salesBillEdit($invoice_id)
      {
        ...
        return view('agent.account.edit',compact('saleInvoice','invoiceItems','customers','accountUnits','propertyStatus'));
	 	  }
      ...
      public function customerList()
      {
          ...
          return view('agent.account.customer', compact('customers'));
      }
}
