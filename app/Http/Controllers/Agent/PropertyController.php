...
class PropertyController extends Controller
{
    ...
    public function type(Request $request)
    {
        ...
        $data['industrialCount'] = Property::where([['type', 'industrial'], ['agent_id', Auth::guard('agent')->user()->id]])->count();
        if ($this->routeName === 'agent.property_inventory.type') {
            $data['url'] = 'agent.property_inventory.create_property';
        } else {
            $data['url'] = 'agent.property_management.create_property';
        }
    ...
    public function index(Request $request)
    {
        ...
        $query = Property::where('agent_id', Auth::guard('agent')->user()->id)
            ...
            ->with([
               ...
               if ($this->routeName === 'agent.property_inventory.properties') {
    ...
    public function create(Request $request)
    {
        ...
        if($this->routeName == 'agent.property_inventory.create_property') {
          ...
          $information['url'] = 'agent.property_management.store_property';
          $information['admin_url'] = 'agent.admin.property_management.store_property';
        }
    ...
    public function store(StoreRequest $request)
    {
        DB::transaction(function () use ($request) {
            ...
            $property = Property::create([
                'property_type' => $this->routeName == 'agent.property_inventory.store_property' ? 'full' : 'partial',
    ...
    public function unitTypeAdd()
    {
        ...
        $view = view('agent.unit-type.add',compact('property_types'))->render();
    ...
    {
        return view('agent.property.view',compact('id'));
    }
    ...
    public function propertyFullWings($property_id)
    {
      $wings = PrtWing::where('property_id',$property_id)->get();
			...
			$view = view('agent.property.full_property_wings',compact('property_id','wings','countmore'))->render();
			return response()->json(['status'=>'success','view'=>$view]);
    }
    ...
    public function propertyFullWingsStore(Request $request)
    {
			...
			try
			{
				...
				DB::commit();
				$view = view('agent.property.full_property.floors',compact('property_id','wing_id','wings','wing_number'))->render();
    ...
    public function propertyFullFloorsStore(Request $request)
    {
			...
			try
			{
			  ...
			  $partailsDetails = PrtUnit::where('wing_id', $wing_id)->where('floor_id', $floor_id)->get()->keyBy('flat_number');
				$view = view('agent.property.full_property.property-details',compact('property_id','wing_id','floor_id','floors','floor_number','budgets','categories','flatcount','partailsDetails','units'))->render();
    ...
    public function propertyFullFloorsPartial(Request $request)
    {
      try{
        ...
        $view = view('agent.property.full_property.property-details',compact('property_id','wing_id','floor_id','floors','floor_number','units','budgets','categories','flatcount','partailsDetails'))->render();
        return response()->json(['status'=>'success','view'=>$view]);
      }catch (\Throwable $e)
			{
        dd($e);
    ...
    public function propertyFloorStatusChanges(Request $request)
    {
        ...
        $view = view('agent.property.floor-status-change',compact('property','wings','floors','property_id','wing_id','floor_id','floor_details','unit_id','property_status','customers'))->render();
        return response()->json(['status'=>'success','view'=>$view]);
    }
    ...
    {
      $properties = Property::with('propertyContent')->where('agent_id', Auth::guard('agent')->user()->id)->where('property_type', 'full')
        ->where('approve_status', 1)
        ->where('status', 1)
        ->get();

      return view('agent.property.manage_status',compact('properties'));
    }
    ...
    public function propertyByFloorDetails(Request $request)
      ...
      ->get();
			...
			$view = view('agent.property.manage_status.floor-details',compact('floors','wings'))->render();
			return response()->json(['status'=>'success','view'=>$view]);
		}
    ...
    public function convertedCustomer()
    {
        ...
        $prtStatus = PrtFloorStatus::with('customer','property','wing','floors','Units','property.propertyContent')->whereNotNull('customer_id')
                  ->whereHas('customer', function ($query) use ($agentId) {
                      $query->where('agent_id', $agentId);
                  })->get();

        return view('agent.property.converted_customer', compact('prtStatus'));
    }
}