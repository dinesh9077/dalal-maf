...
class PropertyController extends Controller
{
    ...
    {
        $data['commertialCount'] = Property::where([['type', 'commercial'], ['user_id', Auth::guard('web')->user()->id]])->count();
        $data['residentialCount'] = Property::where([['type', 'residential'], ['user_id', Auth::guard('web')->user()->id]])->count();
        $data['industrialCount'] = Property::where([['type', 'industrial'], ['user_id', Auth::guard('web')->user()->id]])->count();
        return view('users.property.type', $data);
    }
    public function index(Request $request)
    {
        ...
        $data['properties'] = Property::where('user_id', Auth::guard('web')->user()->id)
    ...
    {
        $property = Property::with('galleryImages')->where('user_id', Auth::guard('web')->user()->id)->findOrFail($id);
    ...
    public function update(PropertyUpdateRequest $request, $id)
    {
        DB::transaction(
            function () use ($request, $id) {
                ...
                $property = Property::where('user_id', Auth::guard('web')->user()->id)->findOrFail($request->property_id);
    ...
    {
        $messages = PropertyContact::with('property')->where('user_id', Auth::guard('web')->user()->id)->latest()->get();
        return view('users.property.message', compact('messages'));
    }
    ...
    {
        $inquiries = PropertyContact::with('property')->where('inquiry_by_user', Auth::guard('web')->user()->id)->latest()->get();

        return view('users.property.inquiry', compact('inquiries'));
    }
}
